...
class spi_host_base_vseq extends cip_base_vseq #(
  ...
  // re-active sequence
  spi_device_extended_seq m_spi_device_seq[SPI_HOST_NUM_CS];
  ...
  virtual task start_reactive_seq();
    ...
    for( int i = 0; i < SPI_HOST_NUM_CS; i++) begin
      m_spi_device_seq[i] = spi_device_extended_seq::type_id::create($sformatf("spi_host[%0d]",i));
  ...
  virtual function void update_spi_agent_regs();
    for (int i = 0; i < SPI_HOST_NUM_CS; i++) begin
      ...
      cfg.m_spi_agent_cfg.fullcyc[i]      = spi_config_regs.fullcyc[i];
      cfg.m_spi_agent_cfg.csnlead[i]      = spi_config_regs.csnlead[i];
    end
    cfg.m_spi_agent_cfg.csid              = spi_host_ctrl_reg.csid;
    cfg.m_spi_agent_cfg.direction         = spi_host_command_reg.direction;
    cfg.m_spi_agent_cfg.spi_mode          = spi_host_command_reg.mode;
    cfg.m_spi_agent_cfg.csaat             = spi_host_command_reg.csaat;
    cfg.m_spi_agent_cfg.len               = spi_host_command_reg.len;
  ...
  virtual task write_data_to_fifo(ref bit [7:0] data_q[$]);
    ...
    while (data_q.size() > 0) begin
      ...
      mask[cnt] = 1'b1;
      if (cnt == 3) begin
        send_tl_access(.addr(wr_align_addr), .data(data), .write(1'b1), .mask(mask),  .blocking(1'b1));
        cnt  = 0;
        data = '0;
        mask = '0;
      end else begin
